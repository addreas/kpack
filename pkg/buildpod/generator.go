...
type BuildPodable interface {
	...
	V1Alpha1Bindings() (buildapi.V1Alpha1Bindings, error)
...
func (g *Generator) fetchServiceBindings(ctx context.Context, build BuildPodable) ([]buildapi.ServiceBinding, error) {
	...
	if services := build.Services(); len(services) != 0 {
		for _, s := range build.Services() {
			var b servicebinding.ServiceBinding
			if s.APIVersion == "v1" && s.Kind == "Secret" {
				b = servicebinding.ServiceBinding{
					Name:      s.Name,
	...
	v1alpha1Bindings, err := build.V1Alpha1Bindings()
	if err != nil {
		return nil, err
	}

	if len(v1alpha1Bindings) != 0 {
		for _, s := range v1alpha1Bindings {
			...
			bindings = append(bindings, &servicebinding.V1Alpha1ServiceBinding{
				Name:        s.Name,
